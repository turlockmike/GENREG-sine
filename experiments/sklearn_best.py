"""
Experiment: sklearn Best Configs

Run all 3 sklearn datasets with best known configurations.
Compare fixed K=4 vs variableK (init=4) to see if variableK discovers better K.
...
Configs:
- Fixed K=4 (proven best)
- VariableK init=4 (to discover optimal K)
...
def load_dataset(name):
    ...
    if name == 'digits':
        data = load_digits()
        H = 32  # Larger hidden for 10 classes
    elif name == 'cancer':
    ...
    return X_train, y_onehot, X_test, y_test, X_train.shape[1], n_classes, H
...
BASE = {
    ...
    'grow_rate': 0.01, 'shrink_rate': 0.01, 'initial_k': 4, 'max_k': 16,
}

CONFIGS = {}

# Generate configs for all datasets × K types × seeds
for dataset in ['digits', 'cancer', 'wine']:
    for k_type in ['fixed', 'variable']:
        for seed in [0, 1000, 2000]:
            name = f"{dataset}_{k_type}_seed{seed}"
            CONFIGS[name] = {
                **BASE,
                'dataset': dataset,
                ...
                'K': 4,
                'seed': seed,
                'description': f"{dataset}, {'variableK init=4' if k_type == 'variable' else 'fixed K=4'}, seed={seed}",
            }
...
def main():
    ...
    print("=" * 60)
    print(f"SKLEARN BEST: {config_name}")
    print("=" * 60)
    print(f"Description: {config.get('description', '-')}")

    # Load data
    X_train, y_onehot, X_test, y_test, input_size, n_classes, H = load_dataset(config['dataset'])
    config['H'] = H

    print(f"Dataset: {config['dataset']}")
    print(f"Data: {len(X_train)} train, {len(X_test)} test, {input_size} features, {n_classes} classes")
    print(f"Architecture: H={H}, K={'variable' if config['arch'] == 'variablek' else config['K']}")

    csv_path = Path("results/live") / f"sklearn_{config_name}.csv"
    json_path = csv_path.with_suffix('.json')
    csv_path.parent.mkdir(parents=True, exist_ok=True)

    with open(json_path, 'w') as f:
        json.dump({'config_name': config_name, 'input_size': input_size,
                   'n_classes': n_classes, **config}, f, indent=2)
