"""
Experiment: VariableK Scaling Test

Hypothesis: VariableK is the best approach, but needs more population and/or
more generations to consistently outperform fixed K.

Test on Interaction problem (requires K≥2, variableK achieved 85.5% vs fixed K=2's 71.2%)
with scaled compute:
...
def load_synthetic_interaction(n_samples=2000, n_features=64, n_informative=4, noise=0.3):
    """
    ...
    for gen in range(generations):
        ...
        if csv_path:
            with open(csv_path, 'a') as f:
                f.write(f"{gen},{acc},{best_fitness},{elapsed:.1f},{info['connections']},{info['mean_k']},{info['min_k']},{info['max_k']}\n")
                f.flush()

        if gen % 100 == 0:
            print(f"    Gen {gen}: {acc:.1%}, conn={info['connections']}, k_mean={info['mean_k']:.1f}, time={elapsed:.0f}s", flush=True)
...
# Configurations for scaling test
# All run 2000 generations - check CSV at gen 1000, 1500, 2000 for comparison
BASE = {
    'H': 16, 'initial_k': 2, 'max_k': 16,
    'generations': 2000,  # Run long, check intermediate results
    'sa_steps': 20, 'seed_fraction': 0.05,
    'weight_rate': 0.02, 'weight_scale': 0.15,
    'index_swap_rate': 0.02, 'grow_rate': 0.01, 'shrink_rate': 0.01,
}

CONFIGS = {
    # Population 100 (baseline) - 3 seeds
    'pop100_seed0': {**BASE, 'pop_size': 100, 'seed': 0, 'description': 'pop=100, seed=0'},
    'pop100_seed1': {**BASE, 'pop_size': 100, 'seed': 1000, 'description': 'pop=100, seed=1000'},
    'pop100_seed2': {**BASE, 'pop_size': 100, 'seed': 2000, 'description': 'pop=100, seed=2000'},

    # Population 200 - 3 seeds
    'pop200_seed0': {**BASE, 'pop_size': 200, 'seed': 0, 'description': 'pop=200, seed=0'},
    'pop200_seed1': {**BASE, 'pop_size': 200, 'seed': 1000, 'description': 'pop=200, seed=1000'},
    'pop200_seed2': {**BASE, 'pop_size': 200, 'seed': 2000, 'description': 'pop=200, seed=2000'},

    # Population 500 - 3 seeds
    'pop500_seed0': {**BASE, 'pop_size': 500, 'seed': 0, 'description': 'pop=500, seed=0'},
    'pop500_seed1': {**BASE, 'pop_size': 500, 'seed': 1000, 'description': 'pop=500, seed=1000'},
    'pop500_seed2': {**BASE, 'pop_size': 500, 'seed': 2000, 'description': 'pop=500, seed=2000'},
}


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--config', type=str, default='baseline_pop100_gen1000')
    ...
    print("=" * 60)
    print(f"VARIABLEK SCALING: {config_name}")
    print("=" * 60)
    print(f"Description: {config.get('description', '-')}")
    print(f"Population: {config['pop_size']}, Generations: {config['generations']}")

    # Load data
    X_train, y_onehot, X_test, y_test = load_synthetic_interaction()
    print(f"Data: {len(X_train)} train, {len(X_test)} test")

    csv_path = Path("results/live") / f"variablek_scaling_{config_name}.csv"
